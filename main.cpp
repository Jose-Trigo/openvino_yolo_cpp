...
int main() {
    ...
    // ---------------- PrePostProcessor: remove manual preprocessing ----------------
    using namespace ov::preprocess;

    PrePostProcessor ppp(model);

    // Input: we will feed raw OpenCV BGR frame as NHWC uint8
    ppp.input()
        .tensor()
            .set_element_type(ov::element::u8)          // uint8 input
            .set_layout("NHWC")                         // HWC from OpenCV
        .preprocess()
            .convert_element_type(ov::element::f32)     // to float32
            .convert_color(ColorFormat::RGB)            // BGR -> RGB
            .resize(ResizeAlgorithm::RESIZE_LINEAR)     // resize to model size
            .scale({255.0f})                            // normalize [0,1]
        .model()
            .set_layout("NCHW");                        // model expects NCHW

    // Output: ensure float32
    ppp.output()
        .tensor()
    ...
    // ---------------- Compile model & create ONE infer request ----------------
    ...
    while (true) {
        ...
        // ---------------- Set input tensor directly from OpenCV frame ----------------
        // Frame is BGR HxWx3 uint8, PPP expects NHWC u8 and will:
        //  - convert to RGB
        //  - resize
        //  - normalize
        ...
        // ---------------- Draw + write (not included in preprocessing) ----------------
